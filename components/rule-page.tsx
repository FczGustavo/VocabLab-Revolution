...
import {
  FolderDeleteChoice,
  FolderDeleteOptions,
} from "@/components/folder-delete-dialog";
...
export function RulePage() {
  ...
  useEffect(() => {
    try {
      setColors(
        JSON.parse(localStorage.getItem("rulelab_folder_colors") ?? "{}"),
      );
    } catch {
      setColors({});
    }
  ...
  const setColor = (id: string, color: FolderColor) => {
    setColors((current) => {
      const next = { ...current, [id]: color };
      localStorage.setItem("rulelab_folder_colors", JSON.stringify(next));
      return next;
    });
  };
  ...
  const activeCards = isReviewSelected
    ? reviewCards.filter(
        (card) =>
          !selectedReviewFolderId || card.folderId === selectedReviewFolderId,
      )
    : cards;
  const reviewParent = folders.find(
    (folder) => folder.id === selectedReviewFolderId,
  );
  const currentName = isReviewSelected
    ? `Review of \"${reviewParent?.name ?? "Rules"}\"`
  ...
  const saveCard = async () => {
    if (editingCard) {
      if (await updateCard({ ...editingCard, front, back }))
        setEditorOpen(false);
  ...
  const saveFolder = async () => {
    if (managerFolder && (await renameFolder(managerFolder.id, managerName)))
  ...
  const performFolderDelete = async () => {
    ...
    if (!(await deleteFolder(managerFolder.id))) {
      setFormError(
        "The cards were processed, but the folder could not be deleted.",
      );
      return;
    }
    setDeleteOpen(false);
  ...
  const transferSelectedCards = async (
    ids: string[],
    ...
    const results = await Promise.all(
      ids.map((id) => {
        ...
        return card
          ? updateCard({ ...card, folderId: targetFolderId })
    ...
    <div className="w-full">
                ...
                {folders
                  ...
                  .map((folder) => {
                    const count = reviewCards.filter(
                      (card) => card.folderId === folder.id,
                    ).length;
                    return (
                      <FolderCard
                        key={`review-${folder.id}`}
                        name={`Review of \"${folder.name}\"`}
                        wordCount={count}
                        isReview
                        gradient="amber"
                        subtitle={`${count} cards to review`}
                        onClick={() => {
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editingCard ? "Edit card" : "Add rule card"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <label className="block text-sm font-medium">
              ...
              <Textarea
                ...
                placeholder="Rule, question or prompt"
              />
            </label>
            <label className="block text-sm font-medium">
              Back
              <Textarea
                ...
                placeholder="Answer, explanation or example"
              />
            </label>
            {formError && (
      ...
      <Dialog
        open={Boolean(managerFolder)}
        onOpenChange={(open) => !open && setManagerFolder(null)}
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-[94vw] overflow-y-auto sm:max-w-md">
          ...
          {managerFolder && <FolderManagerPanel
            name={managerName}
          ...
          <div className="hidden">
              ...
              <div className="flex gap-2">
                {colorOptions.map((color) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      managerFolder && setColor(managerFolder.id, color)
                    }
                    className={cn(
                      "size-8 rounded-full transition-all",
                      colorClass[color],
                      colors[managerFolder?.id ?? ""] === color
            ...
            <div className="space-y-3 border-t border-border/30 pt-2">
              <LongPressButton
                ...
                className="h-10 w-full rounded-md border border-destructive/20 bg-destructive/5 text-destructive transition-colors hover:bg-destructive/10"
              >
            ...
            <div className="border-t border-border/30 pt-3">
              <Button
                variant="outline"
                className="h-12 w-full justify-between rounded-xl bg-muted/20"
        ...
        <AlertDialogContent className="max-w-[92vw] sm:max-w-sm">
            ...
            {folders
              ...
              .map((folder) => (
                <FolderDeleteChoice
                  ...
                  selected={
                    deleteMode === "transfer" && transferTarget === folder.id
                  }
                >
                  {folder.name}
                </FolderDeleteChoice>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => void performFolderDelete()}
              disabled={deleteMode === "transfer" && !transferTarget}
            >
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
  ...
  return (
    <article
      className={cn(
        ...
        layout === "grid" &&
          (squareCards ? "aspect-square h-auto" : "h-[19rem] sm:h-80"),
    ...
    >
      ...
      <div
        className={cn(
          "min-w-0 pr-20",
          layout === "grid" && "border-b border-border/35 pb-3",
        )}
      >
        <h3 className="whitespace-pre-wrap text-xl font-medium leading-snug tracking-tight text-foreground/80">
          {card.front}
        </h3>
      </div>
      {layout === "grid" ? (
        ...
        <p className="line-clamp-2 flex-1 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">
          {card.back}
        </p>
      ) : (
        <p className="mt-2 line-clamp-2 whitespace-pre-wrap text-[13px] leading-relaxed text-muted-foreground">
          {card.back}
        </p>
      )}
    </article>
