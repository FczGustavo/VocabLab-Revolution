...
export function RegencyPage() {
  ...
  const [studyCards, setStudyCards] = useState<RegencyCard[] | null>(null)
  ...
  if (studyKind) return <RegencyStudyMode cards={studyCards ?? activeCards} folderName={currentFolderName} folderId={isReviewFolderSelected ? selectedReviewFolderId : selectedFolderId === "__general__" ? null : selectedFolderId} mode={studyKind} display={{ showCategory, showGrammaticalForm, showMeaning, showContrast, showExample, showTranslation }} onMarkForReview={isReviewFolderSelected ? undefined : addToReviewFolder} onMarkAsLearned={isReviewFolderSelected ? removeFromReviewFolder : undefined} onRecordResult={recordStudyResult} onExit={() => { setStudyKind(null); setStudyCards(null) }} />
  ...
  return (
    <div className="w-full">
      ...
      <Dialog open={studyPickerOpen} onOpenChange={setStudyPickerOpen}><DialogContent className="max-w-sm p-4 sm:p-6"><DialogHeader><DialogTitle>Start study</DialogTitle><DialogDescription>Choose how you want to review this folder.</DialogDescription></DialogHeader><div className="grid gap-2 w-full min-w-0"><Button variant="outline" disabled={!canUseRegencyMultipleChoice(activeCards)} className="h-auto w-full min-w-0 justify-start p-3 sm:p-4 text-left whitespace-normal" onClick={() => { setStudyPickerOpen(false); setStudyCards([...activeCards]); setStudyKind("choice") }}><span className="w-full min-w-0"><span className="block text-sm">Multiple choice</span><span className="mt-1 block text-xs font-normal text-muted-foreground whitespace-normal break-words">{canUseRegencyMultipleChoice(activeCards) ? "Choose the construction that completes each context." : "Requires at least 10 cards and 4 distinct answers."}</span></span></Button><Button variant="outline" className="h-auto w-full min-w-0 justify-start p-3 sm:p-4 text-left whitespace-normal" onClick={() => { setStudyPickerOpen(false); setStudyCards([...activeCards]); setStudyKind("recall") }}><span className="w-full min-w-0"><span className="block text-sm">Active recall</span><span className="mt-1 block text-xs font-normal text-muted-foreground whitespace-normal break-words">Try to remember the pattern before revealing it.</span></span></Button><Button variant="outline" className="h-auto w-full min-w-0 justify-start p-3 sm:p-4 text-left whitespace-normal" onClick={() => { setStudyPickerOpen(false); setStudyCards([...activeCards]); setStudyKind("flip") }}><span className="w-full min-w-0"><span className="block text-sm">Flip cards</span><span className="mt-1 block text-xs font-normal text-muted-foreground whitespace-normal break-words">Turn the card and mark whether you knew it.</span></span></Button></div></DialogContent></Dialog>
