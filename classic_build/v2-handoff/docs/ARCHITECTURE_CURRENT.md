...
4. A sincronização tenta gerar operações pontuais idempotentes (`OPERATION_PROTOCOL = 4`) e enviá-las à tabela `vocablab_sync_operations` no Supabase. O mecanismo continua quebrado no uso real e não possui comprovação E2E de convergência.
...
| `rulelab-db` | 3 | `cards`, `folders`, `theoryDocuments`, `meta` | cards: `createdAt`, `folderId`; folders: `createdAt`; theory: `folderId`, `updatedAt` |
...
Dos seis bancos, cinco guardam conteúdo dos Labs; `vocablab-auto-sync-db` guarda baselines internos de sincronização. Portanto, não deve ser tratado como banco de conteúdo migrável.
...
**[Confirmado em 8 ago 2026]** Suíte em Vitest: 11 arquivos de teste, 43 testes unitários cobrindo:
