...
**[Preservar]** A unicidade técnica no IndexedDB usa o índice `word_pos` sobre `[word, partOfSpeech]`; os fluxos de persistência normalizam `word` para minúsculas antes de gravar. Grafias iguais em classes distintas geram cards distintos. Sentidos diferentes da mesma classe ficam no mesmo card.
...
- `id`: string
- `folderId`: string
- `title`: string (máx 240 caracteres)
- `blocks`: `RuleTheoryBlock[]` (máx 80 blocos)
  - `id`: string
...
**[Compatibilidade]** Em registros antigos, `lab`, `mode`, `mistakeCards`, `totalMistakes`, `cardIds`, `folderId` e `durationSeconds` podem estar ausentes. O importador deve aplicar defaults/fallbacks sem rejeitar a sessão.
