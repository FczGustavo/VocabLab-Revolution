...
export function FolderManagerPanel({ name, onNameChange, onRename, renaming = false, colors, activeColor, onColorChange, cardCount, groupCount, groupLabel = "categorias", onTransfer, onDelete, itemLabel = "cards", transferHint = "Filtrar e mover" }: FolderManagerPanelProps) {
  return <div className="mt-2 space-y-5">
    ...
    <div className="border-t border-border/30 pt-4"><Button variant="outline" className="h-12 w-full justify-between rounded-xl bg-muted/20 px-3 hover:bg-muted/45" onClick={onTransfer}><span className="flex items-center gap-2"><span className="flex size-7 items-center justify-center rounded-lg bg-primary/10 text-primary"><ArrowRightLeft className="size-3.5" /></span>Sistema de transferência</span><span className="text-[11px] text-muted-foreground">{transferHint}</span></Button></div>
