...
- **[Confirmado]** Dados de conteúdo ficam no dispositivo em cinco bancos IndexedDB separados; um sexto IndexedDB mantém baselines internos de sincronização. Preferências e histórico ficam majoritariamente em `localStorage`.
- **[Confirmado]** IA é mediada por rotas servidoras (`/api/ai/*`) via OpenRouter; a chave de API nunca vai ao cliente.
- **[Confirmado no código, quebrado no produto]** O protocolo multiwriter pretende permitir criação e alteração em qualquer dispositivo pareado. A sincronização continua falhando no uso real e não pode ser considerada funcional ou confiável.
