...
8. **Sincronização V8 não será portada**: O motor atual continua quebrado no uso real. O Classic preservará requisitos de multiwriter, offline, tombstones e dispositivos, mas terá implementação e testes E2E novos.

## Decisões pendentes (Requerem definição antes da implementação do Classic)

1. **Stack Tecnológica do Classic**: Definir entre (a) monorepo TypeScript com React Native/Expo no Android e React/Next.js na Web, compartilhando domínio e contratos; ou (b) Kotlin Multiplatform/Compose no Android, com contratos independentes e eventual implementação web separada. Não presumir compartilhamento direto de lógica TypeScript com Kotlin.
...
|---|---|
| Regras lexicais (word + POS) e contextos EN/PT-BR | Armazenamento fragmentado em 5 bancos de conteúdo + 1 banco de baseline de sync |
